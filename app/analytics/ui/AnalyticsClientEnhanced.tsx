...
export default function AnalyticsClientEnhanced({ data: initialData }: Props) {
  ...
  useEffect(() => {
    if (
      !hasUserSelectedRound &&
      data &&
      data.currentRoundMatches &&
      Object.keys(data.currentRoundMatches).length > 0
    ) {
      const rounds = Object.values(data.currentRoundMatches).map(
        (r) => r.round
      );
      if (rounds.length > 0) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3 p-4 rounded-2xl border border-green-400/20 bg-green-900/20 shadow-[0_0_0_1px_rgba(34,197,94,0.08)]">
        <div className="text-sm text-green-400 flex items-center">
          ✨ Analytics do seu Canal | 📊 Dados em tempo real dos palpites da sua
          comunidade
        </div>
      </div>
                ...
                {data.firstGuessers.map((guess, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-zinc-500">{guess.date}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          Object.keys(data.currentRoundMatches).length > 0 ? (
            <div className="space-y-4">
                  ...
                  .map(([key, roundData]) => (
                    ...
                    >
                      ...
                      >
                        (
                        {roundData.openMatches > 0
                          ? `🔴 ${roundData.openMatches} abertas`
                          : `📊 ${roundData.totalMatches} partidas`}
                        )
                      </span>
                    </button>
                  ))}
              ...
              {(() => {
                ...
                const roundData =
                  data && data.currentRoundMatches
                    ? data.currentRoundMatches[roundKey]
                    : null;
                  ...
                  return (
                    <div className="rounded-2xl border border-zinc-700 bg-zinc-900/60 p-12 text-center">
                      ...
                      <div className="text-xl text-zinc-400 mb-2">
                        Rodada {selectedRound} não encontrada
                      </div>
                      <div className="text-zinc-500">
                        Selecione uma rodada disponível
                      </div>
                    </div>
                  );
                }
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-xl border border-amber-400/20 bg-amber-900/10 p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4 text-sm text-amber-400">
                          ...
                          <span>
                            ✅ {roundData.finishedMatches} finalizadas
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      {roundData.matches.map((match) => (
                        <div
                          key={match.id}
                          className={`rounded-2xl border border-amber-400/20 bg-zinc-900/60 p-6 shadow-[0_0_0_1px_rgba(255,196,28,0.08)] transition-all ${
                            isLoading
                        ...
                        >
                            ...
                            {match.result && (
                              <div className="mt-2 p-2 bg-blue-500/10 rounded text-center">
                                <span className="text-blue-300 font-semibold">
                                  Resultado: {match.result.goalsHome} x{" "}
                          ...
                          <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
                            {match.betDistribution.map((bet) => (
                              <div
                                key={bet.type}
                                className="text-center p-2 rounded bg-zinc-800/50"
                              >
                                <div
            ...
            <div className="rounded-2xl border border-zinc-700 bg-zinc-900/60 p-12 text-center">
              ...
              <div className="text-zinc-500">
                As partidas aparecerão aqui organizadas por rodada
              </div>
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-amber-300">
                {data.outcomeDistribution.reduce(
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-300">
                {data.firstGuessers.length > 0
                  ? data.firstGuessers[0].user.slice(0, 10)
                  : "N/A"}
              </p>
            </div>
            <div className="text-3xl opacity-80">🥇</div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-300">
                {data.accuracyRate.length > 0
                  ? `${(
                      ((data.accuracyRate[0]?.value || 0) /
                        (data.accuracyRate.reduce(
                          (sum, item) => sum + item.value,
                          0
                        ) || 1)) *
                      100
